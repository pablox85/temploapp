...
export async function selectItemAction(itemId: string): Promise<ActionState> {
  ...
  if (error) {
    console.error("select_own_item error:", JSON.stringify(error, null, 2)); // TEMPORAL: quitar después de debuggear
    ...
    // TEMPORAL: mostrar código y mensaje real en pantalla para diagnosticar
